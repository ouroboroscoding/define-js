...
import { combine, isObject } from '@ouroboros/tools';
...
export default class Parent extends Base {
	...
	requires(require?: Record<string, string | string[]>): Record<string, string[]> | void {
		...
		const oRequire: Record<string, string[]> = {};
		...
		for(const k of Object.keys(require)) {
			...
			if(typeof require[k] === 'string') {
				if(!(require[k] as string in this._nodes)) {
					throw new Error('__require__[' + String(k) + ']:' + String(require[k]));
				}
				oRequire[k] = [require[k] as string];
			}

			// Else if it's an array type
			else if(Array.isArray(require[k])) {
				for(const s in require[k] as string[]) {
					if(!(require[k][s] as string in this._nodes)) {
